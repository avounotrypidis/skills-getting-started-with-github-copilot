        ...
        if (details.participants.length > 0) {
          participantsHTML = `<ul class="participants-list no-bullets">` +
            details.participants
              .map(
                (email) =>
                  `<li class="participant-item">${email} <span class="delete-icon" title="Remove participant" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(email)}">&#128465;</span></li>`
              )
              .join("") +
            `</ul>`;
        } else {
